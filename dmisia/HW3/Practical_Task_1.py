...
search1 = int(input('Enter your number of line(1-56): '))
search2 = int(input('1) Find words "better","never" and "is".\n'
                    '2) Display all text in uppercase.\n'
                    '3) Replace all occurrences of the symbol "i" with "&"\n'
                    'Enter number of action you need to perform: '))
better = (lines[search1].count("better"))
never = (lines[search1].count("never"))
is1 = (lines[search1].count("is"))
if search2 == 1:
    print('better =', better, 'never =', never, 'is =', is1)
elif search2 == 2:
    print(lines[search1].upper())
elif search2 == 3:
    print(lines[search1].replace('i', '&'))
else:
