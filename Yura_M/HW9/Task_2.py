...
while run:
    ...
    if keys[pygame.K_RIGHT] and (WIDTH_DISPLAY - (COORD_X + WIDTH_RECTANGLE)) > DELTA_STEP:
        COORD_X = COORD_X + DELTA_STEP
    if keys[pygame.K_UP] and COORD_Y > DELTA_STEP:
        COORD_Y = COORD_Y - DELTA_STEP
    if keys[pygame.K_DOWN] and (HEIGHT_DISPLAY - (COORD_Y + HEIGHT_RECTANGLE)) > DELTA_STEP:
        COORD_Y = COORD_Y + DELTA_STEP
